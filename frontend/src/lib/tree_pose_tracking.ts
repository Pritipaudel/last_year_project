  ...
  return {
    processFrame: (landmarks: any[], now: number): TreePoseResult => {
      ...
      if (!activeLeg) {
        ...
        return {
          leftLeg: { isHolding: false, holdSeconds: leftAccumulatedSeconds, isComplete: leftAccumulatedSeconds >= h.target_hold_seconds },
          rightLeg: { isHolding: false, holdSeconds: rightAccumulatedSeconds, isComplete: rightAccumulatedSeconds >= h.target_hold_seconds },
          activeLeg: null,
          isComplete: leftAccumulatedSeconds >= h.target_hold_seconds && rightAccumulatedSeconds >= h.target_hold_seconds
        };
      }
      ...
      if (hipDiff / (torsoHeight || 0.001) > t.hip_levelness_threshold) errors.push('hip_unlevel');
      ...
      const leftComplete = leftAccumulatedSeconds >= h.target_hold_seconds;
      const rightComplete = rightAccumulatedSeconds >= h.target_hold_seconds;

      return {
        leftLeg: { isHolding: isHolding && activeLeg === 'left', holdSeconds: leftAccumulatedSeconds, isComplete: leftComplete },
        rightLeg: { isHolding: isHolding && activeLeg === 'right', holdSeconds: rightAccumulatedSeconds, isComplete: rightComplete },
        activeLeg,
